...
import website_teacher_preference_residual_corrective_training as training


class ResidualCorrectiveTrainingTests(unittest.TestCase):
    def test_fixed_recipe_exactly_matches_stage_6_6(self) -> None:
        self.assertEqual(
            training.fixed_recipe(),
            training.frozen_training.fixed_recipe(),
        )
    ...
    def test_checkpoint_metadata_is_fixed_and_unpromoted(self) -> None:
        ...
        self.assertEqual(payload["pipeline_train_pair_count"], 28)
    ...
    def test_real_v2_loader_reproduces_counts_weights_and_isolation(self) -> None:
        ...
        self.assertEqual(sum(len(group["pairs"]) for group in train_groups), 28)
        ...
        self.assertEqual(
            dataset["summary"]["stage_6_9_residual_corrective_pair_count"],
            4,
        )
        self.assertEqual(hashes, training.EXPECTED_HASHES)

    def test_metrics_report_all_frozen_pair_types(self) -> None:
        ...
        self.assertEqual(metrics["pipeline_train"]["aggregate"]["pair_count"], 28)
        ...
        self.assertEqual(metrics["pipeline_development"]["aggregate"]["pair_count"], 4)
